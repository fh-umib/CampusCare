...
function DetailPanel({ item, isOwner, isAdmin, onStatus }: { item: LostFoundItem | null; isOwner: boolean; isAdmin: boolean; onStatus: (id: string, status: LostFoundStatus, successMessage?: string) => Promise<void> }) {
...
export default function LostFoundPage() {
  ...
  async function updateStatus(id: string, status: LostFoundStatus, successMessage?: string) {
    try {
      setError('');
      const updated = await lostFoundService.updateStatus(id, status);
      setItems((current) => current.map((item) => item.id === id ? { ...item, ...updated, reporterName: updated.reporterName ?? item.reporterName } : item));
      setMessage(successMessage ?? `Report marked ${status}.`);
    } catch (err) {
      setError(getApiErrorMessage(err));
    }
  }
  ...
  const isSelectedOwner = Boolean(selected && selected.userId === user?.id);
  ...
  return (
    <>
      <style>{`
        ...
        .lf-owner-resolve{display:flex;flex-wrap:wrap;gap:.4rem;border-top:1px solid #edf2f7;padding-top:.75rem}.lf-owner-resolve>span{width:100%;color:#64748b;font-size:.6rem;font-weight:800}.lf-owner-resolve button{display:flex;width:100%;min-height:40px;align-items:center;justify-content:center;gap:.35rem;border:0;border-radius:9px;padding:.55rem .7rem;color:#fff;background:#0d9e8a;font:inherit;font-size:.68rem;font-weight:850;cursor:pointer}.lf-owner-resolve small{color:#7b8ba0;font-size:.57rem;line-height:1.45}
